...
import com.blockdustry.building.ArcBlockEntity;
...
// 电弧炮（arc）动画渲染（Mindustry DrawTurret）：1×1 建筑，BER 叠画单一转盘 arc_top，
// 随 aimYaw 旋转、整体后坐力位移；贴图用 Mindustry arc.png（32×32 特斯拉线圈顶）喵
public class ArcBlockEntityRenderer implements BlockEntityRenderer<ArcBlockEntity> {
    private static final ResourceLocation TEX_TOP =
            ResourceLocation.fromNamespaceAndPath(Blockdustry.MODID, "textures/entity/arc_top.png");
    // 1×1 占地：转盘 quad 半宽（以方块中心为原点的 -0.5..0.5）喵
    private static final float HALF = 0.5f;

    public ArcBlockEntityRenderer(BlockEntityRendererProvider.Context ctx) {
    }

    @Override
    public void render(ArcBlockEntity be, float partialTick, PoseStack pose,
        ...
        // 移到 1×1 块顶面中心，先整体绕 Y 旋转（贴图"前"方向校准同 duo）喵
        pose.pushPose();
    ...
    // 平面 quad 铺满 1×1（-0.5..0.5，半宽 HALF），uv 覆盖整张贴图，法线朝上（entityCutout 透明底正常）。
