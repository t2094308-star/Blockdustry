...
// 先放大节点后放用电器也能自动连接：放置建筑后反向扫描周围大型节点（半径 15）让节点主动连上。
// 用事件驱动而非节点持续扫描，避免性能爆炸；与 1×1 的 PowerNodeLinkHandler 互补喵
@EventBusSubscriber(modid = Blockdustry.MODID)
public class PowerNodeLargeLinkHandler {
    @SubscribeEvent
    public static void onPlace(BlockEvent.EntityPlaceEvent event) {
        if (!(event.getLevel() instanceof ServerLevel level)) return;
        if (!(event.getPlacedBlock().getBlock() instanceof BlockdustryBuildingBlock)) return;
        BlockPos pos = event.getPos();
        int r = (int) PowerNodeLargeBlockEntity.LASER_RANGE;
            ...
            for (int dy = -r; dy <= r; dy++) {
                for (int dz = -r; dz <= r; dz++) {
                    ...
                    if (be instanceof PowerNodeLargeBlockEntity node
                            && node.getRenderLinks().size() < PowerNodeLargeBlockEntity.MAX_NODES
                            && node.linkValid(pos)) {
                        node.connect(pos);
                    }
                }
